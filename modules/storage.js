...
var StorageManager = class StorageManager {
  ...
  async createConversation(data) {
    ...
    await this.set(this.stores.conversations, conversation);
    return conversation;
  }
  ...
  async addMessage(conversationId, data) {
    ...
    await this.set(this.stores.messages, message);
    return message;
  }
  ...
  async storeAnswer(conversationId, data) {
    ...
    await this.set(this.stores.answers, answer);
    return answer;
  }
  ...
  async storeScreenshot(conversationId, data) {
    ...
    await this.set(this.stores.screenshots, screenshot);
    return screenshot;
  }
  ...
  async addHistory(data) {
    ...
    await this.set(this.stores.history, entry);
    return entry;
  }
  ...
  async getStats(conversationId) {
    ...
    if (!conversation) return { conversationId, totalQuestions: 0, totalAnswers: 0, appliedAnswers: 0, successRate: '0%', messages: 0, duration: 0 };
    const messages = await this.getMessages(conversationId);
    const answers = await this.getAnswers(conversationId);
...
globalThis.__MCQStorageManager = StorageManager;
