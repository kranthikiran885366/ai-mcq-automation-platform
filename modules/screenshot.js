...
var ScreenshotManager = globalThis.ScreenshotManager || class ScreenshotManager {
  constructor(config = {}) {
    ...
    this.scale = config.scale || 1.25; // lower default for faster capture
  }
  ...
  async captureScreenshot(element = null) {
    try {
      ...
      // Fast path: use browser-level visible tab capture when available.
      const fastCapture = await this.captureVisibleTabViaBackground();
      if (fastCapture) {
        const size = this.estimateSize(fastCapture);
        return {
          ...
          metadata: {
            ...
            capture_method: 'tab_capture'
          }
        };
      }

      // Fallback: create canvas using html2canvas or native canvas API
  ...
  /**
   * Try fast browser-level screenshot via background script.
   */
  async captureVisibleTabViaBackground() {
    ...
    try {
      ...
      if (response?.success && typeof response.dataUrl === 'string' && response.dataUrl.startsWith('data:image/')) {
        return response.dataUrl;
      }
    } catch (error) {
      // Fall back to DOM canvas path silently.
    }
